...
<Warning>
**Connect to your RPC node or node provider's WebSocket**, not the raw Flashblocks stream (`wss://mainnet.flashblocks.base.org/ws`). The raw stream is reserved for infrastructure-to-node syncing.
</Warning>
