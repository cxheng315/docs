...
Flashblocks reduce effective block times on Base from 2 seconds to just **200 milliseconds**, making transactions feel instant. Built in collaboration with [Flashbots](https://www.flashbots.net/), Flashblocks stream sub-blocks during each 2-second window, providing sub-second preconfirmations.
...
Both [rollup-boost](https://github.com/flashbots/rollup-boost) and [op-rbuilder](https://github.com/flashbots/op-rbuilder) are built and maintained by Flashbots, while Base maintains the websocket-proxy and node-reth components.
