...
<Steps>
  ...
  <Step title="Configure Wagmi for Base">
    ...
    <Note>
    WalletConnect requires a project ID from [cloud.walletconnect.com](https://cloud.walletconnect.com). The conditional connector pattern lets the app run without it during development. Set `NEXT_PUBLIC_WC_PROJECT_ID` in your `.env.local` when you are ready to enable WalletConnect.
    </Note>
  ...
  <Step title="Write to a contract">
    ...
    <Note>
    The first chain in the `chains` array is the default for all hook calls that do not specify a `chainId`. Always pass `chainId` explicitly when your contract is deployed on a chain that is not first in the array.
    </Note>
